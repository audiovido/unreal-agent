...
void UAividoHUD::RefreshPrompt()
{
	...
	// The prompt belongs to gameplay only: hide it while the main menu, the
	// pause menu or the conversation panel is up (no stale/overlapping hints).
	if (!GM || GM->IsMenuOpen() || GM->IsConversationOpen())
	{
		PromptText->SetVisibility(ESlateVisibility::Collapsed);
		return;
	}
...
void UAividoHUD::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	// Hide the HUD banner while the pause/main menu is up so no gameplay
	// clutter sits behind the overlay.
	if (BannerBorder)
	{
		AAividoGameMode* GM = GameMode.Get();
		const bool bInGameplay = GM && !GM->IsMenuOpen() && !GM->IsConversationOpen();
		BannerBorder->SetVisibility(
			bInGameplay ? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	}
