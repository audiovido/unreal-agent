...
def _bridge_plugin_sources():
    """Canonical bridge plugin shipped with the product.

    Prefers the repo-local copy (used by the ASSET_Showcase2 mission project)
    over the legacy external demo project, so project creation works on clean
    machines without the old demo checkout.  Returns (bootstrap, plugin)
    paths; callers verify existence.  Only the plugin is used today — the
    bootstrap is generated per project by :func:`_bridge_bootstrap_source`.
    """
    local = ROOT / "assetlib" / "tests" / "ue" / "ASSET_Showcase2"
    local_bootstrap = local / "Content" / "Python" / "init_unreal.py"
    local_plugin = local / "Plugins" / "UnrealAgentBridge"
    if local_bootstrap.exists() and (local_plugin / "UnrealAgentBridge.uplugin").exists():
        return local_bootstrap, local_plugin
    legacy_bootstrap = (
        ROOT.parent / "app" / "AudioVidoLivingCity" / "Content" / "Python"
        / "init_unreal.py"
    )
    legacy_plugin = (
        ROOT.parent / "app" / "AudioVidoLivingCity" / "Plugins"
        / "UnrealAgentBridge"
    )
    return legacy_bootstrap, legacy_plugin


def _bridge_bootstrap_source(uproject_path: Path, listener: Path) -> str:
    """Per-project bridge bootstrap for a freshly created project.

    Faithful to the ASSET_Showcase2 bootstrap pattern: on editor start it
    verifies the running project matches the created project, then loads the
    product's canonical bridge listener (``tools/unreal/ue_listener.py``) on
    TCP 6766.  No machine-specific or legacy paths are embedded.
    """
    uproject = str(uproject_path.resolve()).replace("\\", "/")
    listener_path = str(listener.resolve()).replace("\\", "/")
    return f'''# >>> UNREAL_AGENT_BRIDGE >>> (generated by create_project)
...
_ua_host = "127.0.0.1"
_ua_port = 6766
_ua_expected_project = r"{uproject}"
...
def create_project(project_name: str, destination: str, template="Blank"):
    ...
    listener = ROOT / "tools" / "unreal" / "ue_listener.py"
    if not listener.exists():
        return {"ok": False, "error": f"Unreal bridge listener not found: {listener}"}
    source_bootstrap, source_bridge_plugin = _bridge_plugin_sources()
    ...
    target_bootstrap.write_text(
        _bridge_bootstrap_source(uproject_path, listener), encoding="utf-8"
    )
    bridge_plugin_copied = False
    if (source_bridge_plugin / "UnrealAgentBridge.uplugin").exists():
        shutil.copytree(
            source_bridge_plugin,
            project_root / "Plugins" / "UnrealAgentBridge",
            ignore=shutil.ignore_patterns("Intermediate"),
        )
        bridge_plugin_copied = True
    ...
    return {
        ...
        "template": template,
        "opened": True,
        "bridge_plugin_copied": bridge_plugin_copied,
