...
(function () {
  ...
  /* ============================================================
     ...
     ============================================================ */
  var world = { chip: null, ok: false, crew: null };

  function worldChip(label, cls) {
    var chip = $("worldChip");
    if (!chip) return;
    chip.classList.remove("live", "down", "dispatch");
  ...
  function worldTick() {
    ...
    // Heidi + crew — read-only bridge probe through /api/world/state.
    api("/api/world/state").then(function (r) { return r.json(); }).then(function (w) {
      world.crew = (w && w.ok) ? w : null;
      refreshWorldChip();
    }).catch(function () {
      world.crew = null;
      refreshWorldChip();
  ...
  /* Chip text: during a job it shows the Heidi→worker flow truthfully from
     runtime state; idle it shows the live crew summary. Never faked. */
  function refreshWorldChip() {
    var crew = world.crew;
    var heidiOk = crew && crew.heidi && crew.heidi.present;
    if (S.running && S.jobId) {
      var phase = S.worldPhase || "ACKNOWLEDGE";
      var active = crew && crew.active_worker ? crew.active_worker.name : "worker";
      worldChip("HEIDI · " + phase + " → " + active, phase === "DISPATCH" ? "live dispatch" : "live");
    } else if (world.ok && heidiOk && crew.workers && crew.workers.length) {
      var busy = crew.active_worker;
      worldChip("WORLD · HEIDI + " + crew.workers.length + " WORKERS" + (busy ? " · " + busy.name + " " + busy.state : ""), "live");
    } else if (world.ok) {
  ...
  function finishTask(ok, text, ev) {
    ...
    S.jobId = null;
    S.worldPhase = null;
    setTimeout(refreshWorldChip, 400);
  ...
  function pollCommand(jobId) {
    ...
    api("/api/command/" + encodeURIComponent(jobId)).then(function (r) { return r.json(); }).then(function (j) {
      ...
      if (j.state === "queued" || j.state === "running") {
        ...
        if (j.state === "queued") { S.worldPhase = "ACKNOWLEDGE"; setState("thinking", "Heidi acknowledges…"); }
        else { S.worldPhase = "DISPATCH"; setState("working", "Dispatching to the crew…"); }
        refreshWorldChip();
      ...
      api("/api/command/" + encodeURIComponent(jobId) + "/evidence").then(function (r) { return r.json(); }).then(function (ev) {
        ...
        var ok = j.state === "pass";
        S.worldPhase = ok ? "COMPLETE" : "FAILED";
        refreshWorldChip();
        ...
        S.worldPhase = j.state === "pass" ? "COMPLETE" : "FAILED";
        refreshWorldChip();
